...
public class ApproveCommandTests
{
    ...
    [Fact]
    public async Task RunAsync_WhenApproveIsRateLimitedOnce_RetriesWithBackoffAndSucceeds()
    {
        var approver = new FakePullRequestApprover { RateLimitOnceThenSucceedForPrNumbers = { 1 } };
        ...
        Assert.Equal(0, exitCode);
        Assert.Equal(2, approver.ApproveCalls.Count);
    ...
    [Fact]
    public async Task RunAsync_WhenApproveKeepsBeingRateLimited_GivesUpAfterMaxBackoffAndStopsTheBatch()
    {
        ...
        Assert.Equal(3, approver.ApproveCalls.Count);
        ...
        Assert.Contains("Done: 0 approved, 0 skipped, 2 failed", console.Output);
    }
        ...
        int? maxBackoffSeconds = null)
    {
        ...
        var command = new ApproveCommand(restClient, graphQlClient, approver, console, auditLog ?? new FakeAuditLog(), config, delay);
