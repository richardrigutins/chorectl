...
- **GraphQL**, via a hand-written `HttpClient` wrapper (`GitHub/GraphQlClient.cs`, `Queries.cs`) posting hard-coded query strings to `https://api.github.com/graphql` (or the configured GHE host's `/api/graphql`, see below) with `System.Text.Json` deserialization, rather than the official `Octokit.GraphQL` package (still in beta).
...
### Multi-host: github.com or a GitHub Enterprise Server instance

`ChorectlConfig.GitHubHost` (the `github_host` config key) picks the target host; `GitHub/GitHubHost.cs` is the single place that turns it into URLs - `null`/`""`/`"github.com"` all mean "use Octokit's own github.com default", anything else builds `https://{host}/api/v3/` for REST and `https://{host}/api/` for GraphQL (GHE's documented API layout). `CompositionRoot.RunAsync` resolves this lazily, same as every other config value, so the target `IGitHubClient` and the GraphQL `HttpClient` are both registered as DI factories rather than eagerly-built instances - `--help` must still never load config (see Authentication below).

`GitHub/OctokitReleaseSource.cs` (chorectl's own update check) is the one exception: it's hardcoded to `richardrigutins/chorectl` and always targets github.com, ignoring `GitHubHost` entirely - a GHE instance has no relationship to chorectl's own releases, and a token scoped to a GHE host wouldn't authenticate against github.com anyway. `CompositionRoot.BuildReleaseSource` builds it a wholly separate `GitHubClient`/authenticator from the target one, at the cost of an extra `gh auth token` call when the target host already is github.com (the common case) - the simplest option that's correct in every case, not worth optimizing further unless that overhead turns out to matter in practice.

### Authentication: delegated to `gh`

`chorectl` requires the GitHub CLI (`gh`) to already be installed and authenticated. `GitHub/GhCliAuthenticator.cs` shells out to `gh --version` (minimum version check), `gh auth status`, and `gh auth token` - there is no separate login flow or credential storage of chorectl's own. When targeting a non-default host, `--hostname <host>` is appended to the `auth status`/`auth token` calls (never to `--version`, which is host-agnostic) - the user is still responsible for having run `gh auth login --hostname <host>` against that instance first, chorectl never drives `gh auth login` itself. The token is fetched **lazily**: `CachingGitHubAuthenticator` wraps the real authenticator in a `Lazy<string>`, so the underlying `gh` calls only happen the first time a command actually needs a GitHub client, and only once per process no matter how many clients (Octokit's `GitHubClient` via `GitHubCredentialStore`, and the GraphQL `HttpClient` via `GitHubAuthenticationHandler`) end up sharing it. This is why `chorectl --help` never touches `gh` at all.
